...
class 상위클래스{
    //생성자 : 생성자 코드가 없으면 컴파일시 자동으로 디폴트생성자 생성
}

class 하위클래스A extends 상위클래스{
    //생성자
    하위클래스A(){
...
class  하위클래스C extends 상위클래스C{
    하위클래스C(){} //상속클래스C에서 디폴트생성자 존재하므로 supper()자동으로 호출
}
...
public class Step2 {
    public static void main(String[] args) {
        ...
        //E e2=(E)a1;                     //실행하면 오류: a1 객체의 태생은 B(); B는 E를 가지고있지 않음
        //D d2=(D)a2;                     // 실행하면 오류 a2 객체의 태생은 C(); C는 D를 가지고 있지 않음
        
        // --- 본래의 태생 타입을 아는 방법
        boolean result1= a1 instanceof A; //a1객체의 타입은 A클래스가 존재하는지??
